...
* Requires `phoenix_live_view ~> 1.1` (and Phoenix 1.8+). The filter controls ship a colocated runtime hook, which needs both. Apps with a Content Security Policy have to pass a nonce for it.
...
* Clearing a filter now also clears date and number inputs holding a partially entered value. Such an input reads back as empty, so the filter was already empty server-side and no diff ever reached the browser — the entered parts stayed visible after clicking the clear button. The clear buttons now empty their inputs directly, which also means the inputs clear on click instead of a round trip later.
